...
class TestBuildPLGARing(unittest.TestCase):
    def test_build_PLGA_ring(self):
        ...
        reaction = AllChem.ReactionFromSmarts(
            "[I:1][O:2].[I:3][C:4]>>[C:4][O:2].[I:3][I:1]"
        )
        terminal = "hydroxyl"
...
class TestBlockinessPLGA(unittest.TestCase):
    def test_blockiness_PLGA(self):
        ...
        sequence_L = "LLLLLLLL"
        blockiness_L, block_length_G_2, block_length_L_2 = demo.blockiness_PLGA(
            sequence_L
        )
...
class TestPLGABuild(unittest.TestCase):
    def test_plga_build(self):
        ...
        solv_system = x.solvate_system(
            resid_monomer=0.5, salt_concentration=0.1 * unit.mole / unit.liter
        )
