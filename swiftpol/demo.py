...
def build_PLGA_ring(
    ...
    ring_smiles = [
        "O1C(=O)C[I+][I+]OC(=O)C1",
        "C[C@@H]1[I+][I+]OC(=O)[C@H](C)OC1=O",
        "C[C@@H]1O[I+][I+]C(=O)[C@@H](C)OC1=O",
        "C[C@H]1O[I+][I+]C(=O)[C@H](C)OC1=O",
    ]
    ...
    polymer = (
        Chem.MolFromSmiles("C[C@H](O)C(=O)O[C@H](C)C(=O)O[I+]")
        if sequence[0:2] == "LL"
        else Chem.MolFromSmiles("[I+]OC(=O)COC(=O)CO")
    )
    ...
    for i in range(0, len(sequence) - 1, 2):
        if sequence[i + 2 : i + 4] == "LL":
            ...
            raise AttributeError(
                "Check sequence. Input format is LLGG (blocks of dimers) and sequence entered is "
                + sequence
            )
    ...
    if terminal == "hydroxyl":
        polymer = Chem.ReplaceSubstructs(
            polymer, Chem.MolFromSmarts("[OH]"), Chem.MolFromSmiles("O")
        )[0]
        Chem.AddHs(polymer)
    elif terminal == "carboxyl":
        polymer = Chem.ReplaceSubstructs(
            polymer, Chem.MolFromSmarts("[OH]"), Chem.MolFromSmiles("OC(=O)[OH]")
        )[0]
    elif terminal == "ester":
        polymer = Chem.ReplaceSubstructs(
            polymer, Chem.MolFromSmarts("[OH]"), Chem.MolFromSmiles("OC")
        )[0]
    else:
        raise ValueError(
            "terminals accepts one of 3 arguments - hydroxyl, carboxyl or ester"
        )
    polymer = Chem.ReplaceSubstructs(
        polymer, Chem.MolFromSmarts("O[I]"), Chem.MolFromSmarts("[O]")
    )[0]
    Chem.SanitizeMol(polymer)
...
def calculate_box_components_PLGA(
    chains,
    sequence,
    monomers=["OC(=O)CO", "C[C@@H](C(=O)[OH])O"],
    salt_concentration=0.1 * unit.mole / unit.liter,
    ...
    water_to_add = int(
        round((solvent_mass - nacl_mass) / water_mass).m_as(unit.dimensionless).round()
    )
...
class PLGA_system:
    ...
    from swiftpol.demo import (
        build_PLGA_ring,
        blockiness_PLGA,
        calculate_box_components_PLGA,
    )
    from swiftpol.build import PDI
    from swiftpol import build
    from rdkit.Chem import AllChem

    gen_rxn = AllChem.ReactionFromSmarts(
    ...
    def __init__(
        ...
        def spec(
            sequence, blockiness
        ):  # Define limits of lactide percentage and blockiness from input
            actual_lac = (sequence.count("L") / len(sequence)) * 100
            return (
                actual_lac > perc_lactide_target * 0.90
                and actual_lac < perc_lactide_target * 1.10
                and blockiness > blockiness_target * 0.90
                and blockiness < blockiness_target * 1.10
            )
        ...
        for x in range(num_chains):
            ...
            sequence = reduce(
                ...
                np.random.choice(
                    ...
                    p=[perc_lactide_target / 100, 1 - (perc_lactide_target / 100)],
                ),
            )
    ...
    def solvate_system(self, resid_monomer, salt_concentration):
        ...
        molecules, number_of_copies, topology, box_vectors, resid_monomer_actual = (
            calculate_box_components_PLGA(
                chains=self.chains,
    ...
    def build_bulk(self, resid_monomer, salt_concentration=0 * unit.mole / unit.liter):
        ...
        solute_length = max(
            _max_dist_between_points(self.chains[i].to_topology().get_positions())
            for i in range(len(self.chains))
        )
        box_vectors = UNIT_CUBE * solute_length
        bulk_system = pack_box(
            molecules=self.chains,
...
def build_starting_polymer(
    ...
    activation_reaction=AllChem.ReactionFromSmarts(
        "[C:1]=[C:2].[Cl:3].[I:4]>>[C:1](-[I:4])-[C:2]-[Cl:3]"
    ),
):
    """
    ...
    polymer = build.build_polymer(
        sequence=sequence, monomer_list=monomers, reaction=polymer_reaction
    )
    ...
    starting_polymer = activation_reaction.RunReactants(
        [polymer, activatorCl, activatorI]
    )[0][0]
    Chem.SanitizeMol(starting_polymer)

    starting_polymer = activation_reaction.RunReactants(
        [starting_polymer, activatorCl, activatorI]
    )[0][0]
    Chem.SanitizeMol(starting_polymer)
...
def build_crosslinked_polymer(
    ...
    reaction_templates=[
        (
            "[Cl:1]-[*:2]-[*:3](-[I:4])-[*:7].[Cl:8]-[*:9]-[*:10](-[I:11])-[*:12].[F:15].[F:16]>>[*:12]-[*:10](-[F:15])-[*:9]-[*:2]-[*:3](-[F:16])-[*:7].[Cl:1].[I:4].[Cl:8].[I:11]",
            (Chem.MolFromSmiles("F"), Chem.MolFromSmiles("F")),
        ),
        (
            "[*:2]-[*:4](-[F:5])-[*:6]-[*:7]-[*:8](-[F:9])-[*:10].[Cl:13]-[*:14]-[*:15](-[I:16])-[*:17].[Br:20]>>[*:2]-[*:4](-[Br:20])-[*:6]-[*:7]-[*C:8](-[*:10])-[*:14]-[*:15](-[I:16])-[*:17].[Cl:1].[F:9].[F:5]",
    ...
    reactions = [
        (AllChem.ReactionFromSmarts(smarts), reactants)
        for smarts, reactants in reaction_templates
    ]
    ...
    while successful_reactions < num_iterations:
        ...
        action = random.choices(
            ...
            weights=[
                ...
                probability_of_crosslinked_addition,
            ],
            k=1,
        )[0]
        ...
        if action == "crosslink":
            ...
            chosen_polymer = random.choice(
                polymer_history[:-1]
            )  # Select from earlier polymer versions
            chosen_reaction, chosen_reactants = random.choices(
                reactions, weights=[0.0, 0.3, 0.35, 0.35], k=1
            )[0]
            reactant_sequence = (
                polymer_network,
                chosen_polymer,
            ) + chosen_reactants  # Use current and chosen polymer
            ...
            reactant_sequence = (
                polymer_network,
                starting_polymer,
            ) + chosen_reactants  # Add chain to current polymer
    ...
    return replace_halogens_with_hydrogens(
        polymer_history[-1]
    )  # Return last polymer in the history
